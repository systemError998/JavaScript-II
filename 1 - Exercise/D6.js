...
const piuUno = randArr.map((number) => {
  return number+randArr.length
})
console.log(piuUno);

/* ESERCIZIO 7 (map)
  Scrivi una funzione che, dato un array di stringhe, ritorni un nuovo array contenente le lunghezze delle rispettive stringhe 
  dell'array di partenza
  es.: ["EPICODE", "is", "great"] => [7, 2, 5]
*/

stringArray = ["Epicode", "is", "fun"]
const lenght = stringArray.map((word) => {
  return word.length
})
console.log(lenght);
...
function oddElements() {
  ...
  } return oddArr// console.log(oddArr);
}
...
/* for (let i=0; i<movies.length; i++) {
  movie.push(movies[i].Year)
  oldMovie = (Math.min(...movie))
} console.log(`Il film più vecchio è del ${oldMovie}`) */
